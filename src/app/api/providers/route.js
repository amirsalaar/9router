...
export async function POST(request) {
  try {
    ...
    // Bedrock's `profile`, where the credential lives in the local AWS config and no key exists
    // to paste. Without this, following such a provider's own setup notice returns 400.
    const apiKeySubstitute = AI_PROVIDERS[provider]?.apiKeyOptionalWith;
    const hasApiKeySubstitute = !!(
      apiKeySubstitute && body.providerSpecificData?.[apiKeySubstitute]
    );
